...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    `;
    const [rows] = await connection.execute<mysql.RowDataPacket[]>(checkSql, [student_id]);
    ...
    const insertPatientRecordSql = `
      ...
      VALUES (?)
    `;
    const [patientRecordResult] = await connection.execute<mysql.OkPacket>(insertPatientRecordSql, [student_id]);
